...
public class DownloadInvoiceTests : BaseTest
{
    ...
    [Test]
    [Description("24 - Скачивание инвойса после оформления заказа")]
    public async Task DownloadInvoice_AfterOrder_FileIsDownloaded()
    {
        ...
        var invoicePath = await PaymentPage.DownloadInvoiceAsync();
        Assert.That(File.Exists(invoicePath), Is.True, $"Файл инвойса не скачан — отсутствует по пути '{invoicePath}'");
        Assert.That(new FileInfo(invoicePath).Length, Is.GreaterThan(0), "Файл инвойса скачан, но пустой");
    }
}
